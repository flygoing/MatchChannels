...
contract EtherShips is Players, ECTools {
    ...
    function openChannel(bytes32 _merkleRoot, string _webrtcId, uint128 _amount, address _signAddress) public payable {
        ...
        players[msg.sender].balance += uint128(msg.value) - _amount;
        Channel storage c = channels[_channelId];
        (c.p1, c.stake, c.p1root, c.blockStarted) = (msg.sender, _amount, _merkleRoot, uint128(block.number));
    ...
    /// @param _signAddress Opens channel with one MetaMask address, sign state with anoother
    function joinChannel(uint _channelId, bytes32 _merkleRoot, string _webrtcId, uint128 _amount, address _signAddress) public payable {
        require(_channelId < channels.length);
        require((channels[_channelId].p1 != address(0)) && (channels[_channelId].p2 == address(0)));
        require(bytes(players[msg.sender].username).length > 0);
        require(_amount <= players[msg.sender].balance + uint128(msg.value));
        ...
        (c.p2, c.p2root, c.blockStarted, c.balance) = (msg.sender, _merkleRoot, uint128(block.number), _amount + c.stake);
        // update player stats
    ...
    function closeChannel(uint _channelId, bytes _sig, uint _numberOfGuesses, bytes32[35] _paths, uint[5] _pos, uint[5] _nonces) public {
        ...
        uint128 wonAmount = uint128(c.stake * _numberOfGuesses / 5);
        ...
        if (c.halfFinisher != address(0)) {
            ...
            c.blockStarted = uint128(block.number);
    ...
    function _addBalanceToPlayer(address _playerAddr, uint128 _amount, uint _channelId) private {
